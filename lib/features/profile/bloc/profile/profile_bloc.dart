...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  void _changeUser(ChangeUserEvent event, Emitter<ProfileState> emit) async {
    ...
    if (accessToken == null) {
      ...
      final user = await _authorizationRepository
          .changeUser({"name": event.name}, accessToken);
  ...
  void _getUserTickets(
      GetUserTicketsEvent event, Emitter<ProfileState> emit) async {
    final accessToken = await _tokenLocalDatasource.getToken();
    if (accessToken == null) {
      emit(ProfileFailure(error: 'Account error'));
    } else {
      final userTickets =
          await _authorizationRepository.getUserTickets(accessToken);
      if (userTickets["success"] == false) {
        ...
        List<Ticket> ticketsFilter = tickets
            .where((ticket) => ticket.date.isAfter(DateTime.now()))
            .toList();
