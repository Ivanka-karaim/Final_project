...
class SignInBloc extends Bloc<SignInEvent, SignInState> {
  ...
  SignInBloc(this._authorizationRepository, this._tokenLocalDatasource)
      : super(SignInInitial()) {
    on<PhoneNumberEvent>(_phoneNumberCheck);
    on<AuthorizationEvent>(_passwordCheck);
  }

  void _phoneNumberCheck(
      PhoneNumberEvent event, Emitter<SignInState> emit) async {
    ...
    if (phoneNumberValid == false) {
      ...
      final result =
          await _authorizationRepository.authorization(event.phoneNumber);
  ...
  void _passwordCheck(
      AuthorizationEvent event, Emitter<SignInState> emit) async {
